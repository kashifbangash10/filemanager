...
import com.nextguidance.filesexplorer.filemanager.smartfiles.misc.PackageManagerUtils;
...
*/
	...
	private void includeAppFromProcess(MatrixCursor result, String docId, AndroidAppProcess processInfo, String query ) {
		...
		try {
			...
			CharSequence label = appInfo.loadLabel(packageManager);
            displayName = label != null ? label.toString() : process;
		} catch (Exception e) { }
    ...
    private boolean isSystemApp(ApplicationInfo appInfo) {
        // If it's NOT a system app, it's definitely a User App
        if ((appInfo.flags & ApplicationInfo.FLAG_SYSTEM) == 0) {
            return false;
        }
        
        // If it IS a system app, but has been updated by user, it's a User App
        if ((appInfo.flags & ApplicationInfo.FLAG_UPDATED_SYSTEM_APP) != 0) {
            return false;
        }
        
        // If it IS a system app, but has a Launcher Activity (e.g. YouTube, Maps), treat as User App
